...
export default function ExploreJobs(props) {
        ...
        useEffect(() => {
                ...
                /api/jobs?title=${title}&min=${min}&max=${max}&delivery=${delivery}&tags=${tags}&country=${country}&language=${language}&sort=${sort}&pageNumber=${pageNumber}
        ...
        console.log(title);
        // history.push(`/explore-jobs/title/${Title || 'all' }/tags/${ TAGS.length > 0 ? TAGS.toString().replaceAll(",","+") : 'all' }/min/${Min || 0}/max/${Max || 0}/delivery/${Delivery || 0}/country/${Country || 'all'  }/language/${Language || 'all'  }/sort/${Sort || 'rating'}/pageNumber/${PageNumber || 1}`)
        history.push('/explore-jobs')
